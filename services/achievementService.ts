...
export async function checkAndGrantBadges(userId: string): Promise<string[]> {
  ...
  // Get counts
  const [rankRes, reviewRes, followingRes, followerRes, partyRes, pollRes, listRes, bookRankRes] = await Promise.all([
  ...
  const rankCount = (rankRes.count ?? 0) + (bookRankRes.count ?? 0);
