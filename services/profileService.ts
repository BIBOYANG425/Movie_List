...
export async function searchProfilesViaRest(
  accessToken: string,
...
export async function exactUsernameViaRest(
  accessToken: string,
  currentUserId: string,
  username: string,
): Promise<ProfileRow[] | null> {
...
export function buildFallbackAvatar(seed: string): string {
  const encoded = encodeURIComponent(seed);
  return `https://api.dicebear.com/8.x/thumbs/svg?seed=${encoded}`;
}

export function profileAvatarUrl(row: ProfileRow): string {
...
export function manualMediaKey(title: string, year?: string | number | null): string {
  const slug = title
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
export function toTier(value: string | null | undefined): Tier | null {
...
export function toRankingEventType(value: string): 'ranking_add' | 'ranking_move' | 'ranking_remove' | null {
