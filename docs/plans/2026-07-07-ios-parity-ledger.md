...
- MOVIE SEARCH: shipped on branch `fix/movie-search-fuzzy` (PR pending) — see [C4→shipped] audit-findings entry above for full disposition. Web 382 vitest / iOS 381 swift; no migrations.
- **iOS ceremony re-rank event deviation (from final review):** when the ceremony's upsert path (B5) moves an item to a new tier, only the TARGET tier is spliced and compacted — the source tier keeps a gap until its next write and self-heals then. The emission is `ranking_add`, not `ranking_move`. This is strictly better than the pre-branch hard-fail (23505 on duplicate position), but it deviates from the `ranking_move` MUST in the contract. Fix in the iOS management-UI sub-plan: pre-read the row's current tier, compact the source tier, then emit `ranking_move`. Tracked in `docs/contracts/shared-payloads.md` Known deviations §3.
