...
| C7 | Smaller items | web half in PR `fix/c7-web-blocking` (HEAD 1208d98); iOS half pending | — | `fix/c7-web-blocking` | — |
...
- [C6-iOS] [shipped] LocaleStore + L10n: `LocaleStore` (enum, pure `resolve`, `current` getter with first-entry persist, `@AppStorage`-compatible raw-string slot `spool_locale`); `L10n.t(key)` / `L10n.t(key, replacements:)` with zh→en→key fallback chain; 423-key EN + ZH tables (83 web-reused zh values cited inline; 340 new zh values for iOS-only and adapted strings — owner-reviewable in PR body). `L10nParityTests`: bidirectional key parity + no-empty-values + no em/en dash in zh values. `LocalePlumbingTests`: device-default zh seed, persist-once semantics, toggle round-trip. Baseline 741 iOS tests; final 782 (+41: parity + plumbing + locale-wired screen logic).
...
- [C6-iOS] [binding note — NOT an iOS ship] TMDB id allowlist (web B4): the `tmdb_`/`tv_` ALLOWLIST lives in web `fetchLocalizedTitle` (`services/tmdbService.ts`, merged in PR #48). iOS has NO localized-title fetch path — the display twin is deferred (next entry) — so there is nothing to guard on iOS yet. Any future iOS twin MUST bake in the same allowlist (plan Global Constraint; allowlist, not an `ol_`-only denylist). [Final review 2026-07-10: prior entry falsely claimed this shipped on iOS — corrected.]
...
- [C6-iOS] [deferred] `locale()` glue untested — the static `TMDBService.locale()` / `SuggestionsClient.locale()` entry points (the one-line `LocaleStore.current` read) have no direct test; only the pure `locale(for:)` mapping and the store's `readCurrent` path are pinned. Low risk (one-line glue). T2-review rider.
- [C6-iOS] [deferred] Duplicated `.zh → zh-CN` mapping — `TMDBService.locale(for:)` and `SuggestionsClient.locale(for:)` are identical one-liners; a single `SpoolLocale.tmdbLanguage` property would consolidate. T2-review rider; C7-smalls candidate (also unused private `L10n.table(for:)` dead code can drop with it).
...
zh on iOS. 4 tasks, plan at `docs/plans/2026-07-10-c6-ios-zh-plan.md` (committed `eb0eab9`). Baselines: 741 iOS tests. Final: 782 iOS tests (+41: 31 `L10nParityTests` + 10 `LocalePlumbingTests`; the sweep updated existing screen-logic pins in place).

**What shipped (task numbering per the plan doc):**

- **T1 (LocaleStore + L10n mechanism, `6ac228d` + fix `7f3b098`):** `SpoolLocale` (`en`/`zh`, `from(languageCode:)`, first-entry `deviceDefault(preferredLanguages:)`). `LocaleStore` (pure `resolve`, `current` getter persists device-default on first read, `storageKey = "spool_locale"`). `L10n.t(key)` / `L10n.t(key, replacements:)` — zh→en→key fallback, injectable-table overload for real fallback coverage. `L10nParityTests`: bidirectional key parity, no empty values, no em/en dash in zh, placeholder-set parity. Tests 741 → 772.
- **T2 (Locale plumbing + Settings toggle + root `.id` re-render, `030db14`):** `TMDBService.locale()` + `SuggestionsClient.locale()` re-sourced to `LocaleStore.current` (duplicated device-reads killed); in-app toggle re-sources TMDB/OL search locale on the next fetch. `languageSection` in `SettingsScreen` — EN/中文 paper-capsule chips, `@AppStorage(LocaleStore.storageKey)` bound with `LocaleStore.current.rawValue` default. `SpoolAppRoot` `@AppStorage` on the same slot + `.id(rawLocale)` on the content view = full tree re-render on locale switch. `LocalePlumbingTests`: pure mapping both clients, device-default seeding, persist-once, toggle round-trip. Tests 772 → 782.
- **T3 (App copy sweep + table population, `69d4c9a` + fix `26782d0`):** 423-key `EN.table` + `ZH.table` (83 web-reused zh cited inline; 340 new iOS zh — owner-reviewable). All user-visible call sites across 20+ screens/components wired through `L10n.t`; fix round restored uppercase chrome rendering (`.uppercased()` at call sites / helpers, EN byte-parity vs pre-branch) and the notifications lowercase title. No hard-coded user-visible EN strings in zh-targeted code paths (designed EN-stays: ticket-design chrome, debug/NSLog, test-pinned formatters, demo fixtures, concatenated narrative fragments).
- **T4 (Docs, `39ed0cd`):** this ledger section + C6 row flipped COMPLETE.
...
**Owner-reviewable items (PR body carries the 340-entry new-zh table for review):**

- 340 new zh strings for iOS-only and adapted surfaces — confirm register + voice per entry.
- Device-default behavior: fresh install on a zh-primary device opens in zh; en-primary opens in en.

**Known deferred items:**

- Localized-title display twin: shelf items show prior locale's title until next load after a toggle — product question (re-fetch vs natural reload). NOT built on iOS at all yet; when built, it MUST carry the web B4 `tmdb_`/`tv_` allowlist (no iOS `fetchLocalizedTitle` exists today — the allowlist ships web-side only, PR #48).
...
- `locale()` static-glue untested + duplicated `.zh → zh-CN` mapping across the two clients (`SpoolLocale.tmdbLanguage` would consolidate; unused private `L10n.table(for:)` dead code drops with it): T2-review riders, C7-smalls candidates.
...
### Web half (Tasks 1–3, HEAD 1208d98)
...
Migration `20260711_achievements_rls_lockdown.sql` revokes INSERT from
authenticated users; anon already excluded. Migration
`20260711_grant_achievements_rpc.sql` installs the SECURITY DEFINER
`grant_achievements()` function (15 rules, thresholds-verified against
`ACHIEVEMENT_DEFINITIONS`; RETURNING-CTE idempotency). Client
`services/achievementService.ts` `checkAndGrantBadges` swapped from direct
INSERT to `rpc('grant_achievements')` fire-and-forget. `AchievementsView`
`isOwnProfile` default footgun addressed (defaults to `false` — no phantom
own-profile badge writes for cross-user views).
...
- **`notifications` INSERT `WITH CHECK (true)`:** the bell-table still has a
  wide-open INSERT CHECK — same B2 class (bell-abuse class). Deferred sweep
...
4. **Stage-A shelf-notes emission gap** (iOS management UI deferred item):
   C7-iOS scope (iOS emissions carry `{year?}` only, no `notes`).
...
- `notifications` INSERT `WITH CHECK (true)` (bell-abuse class) — deferred sweep
