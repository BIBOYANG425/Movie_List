...
- [C6] [blocking] B1 language toggle hidden behind `hidden md:flex` — unreachable on mobile — fixed on `fix/c6-zh-web-blocking`: toggle moved into the mobile bottom-nav bar (always-visible, all pages inside `AppLayout`); TAIL: toggle absent on Landing/Auth/Public pages (outside `AppLayout`) — deferred.
...
- [C6] [blocking] B4 `useLocalizedItems` called the TMDB localization proxy for `ol_` book ids — books never localize; early-return added for `ol_`-prefixed ids, closing the proxy-quota burn and the D5 deferred from C5.
...
- [C6] [adjudication] iOS zh toggle = `SettingsScreen` row (NOT the web header placement) — owner-reviewable; the web mobile placement (bottom-nav) does not translate literally to iOS nav conventions.
...
zh correctness before the iOS port. 5 blocking findings fixed; no migrations (client-only). Baseline: 533 vitest tests. Final: 533 tests (parity test added in Task 1; net adds offset removed dead expectations).

**B1-B5 dispositions:**

- **B1** language toggle unreachable on mobile (`hidden md:flex` in `AppLayout`): moved into the always-visible mobile bottom-nav bar via a dedicated icon button matching the app's visual language. Reachable on every page inside `AppLayout`. TAIL DEFERRED: Landing, Auth, and Public pages (outside `AppLayout`) have no toggle placement yet — owner decides.
...
- **B5** mixed-language sentences in zh mode: media-type enum values were interpolated as EN string literals directly into zh toast strings (e.g., "已添加到 Movie 列表"). All affected toast paths now route through `t()` with translated enum variants; no EN fragment appears in any zh-mode user-visible string.
...
- Toggle placement: a row in `SettingsScreen` (NOT the web header or mobile bottom-nav placement — iOS nav conventions differ). Owner confirms or redirects in the C6-iOS plan.
