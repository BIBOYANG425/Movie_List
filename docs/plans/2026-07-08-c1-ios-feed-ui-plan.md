...
- Reaction types fixed: `love, fire, laugh, sad, mind_blown` (stamp icons per spec §2). (Correction: contract set is fire/agree/disagree/want_to_watch/love; the spec's laugh/sad/mind_blown was an authoring error from the mockup — see ledger.) Comment composer ≤500 chars, `CommentError.empty/.tooLong` surfaced inline (not toasts).
